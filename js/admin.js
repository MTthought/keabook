    ...
    //console.log(post);
      ...
      <img class="postImg mt10" src="apis/api-download.php?file=${post.image}" alt="${post.image}">`;
       ...
       if(post.user_id){
         post_dom +=`
         <p>
         <button class="btnEditPost mt10">Edit</button>
         <button class="btnDeletePost mt10">Delete</button>
         </p>
         </div>
         `;

       }else{
         post_dom +=`<p><button class="btnDeletePost mt10">Delete</button></p></div>`
       }
...
$(document).on("click", ".btnDeletePost", function() {
  var sPostId = $(this).parent().parent().attr('data-postId');
  $.ajax({
    url: "apis/api-hide-post.php",
    method: "POST",
    data: {sPostId}
  }).done(function(sData){
    console.log(sData);
...
$(document).on("click", ".btnEditPost", function() {
  var sPostId = $(this).parent().parent().attr('data-postId');
  var sPostMsg = $(this).parent().siblings('.postMsg').first().text();
  var editableMsg = $(this).parent().siblings('.postMsg')
